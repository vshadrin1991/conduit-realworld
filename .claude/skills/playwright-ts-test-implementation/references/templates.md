...
- Arrange with `get(ConduitRestClient).api.<domain>.create()`, act with an endpoint helper, assert the returned model with `toMatchObject`.
- Negative case: `get(ConduitRestClient, { guest: true }).response({ name, path, pathData, method, body, statusCode })`, then assert `(await response.json()) as ErrorResponse`.
- A new slug/id created outside a flow: `get(ConduitRestClient).api.articles.track(slug)`.
...
- Non-element component (browser/session level, like `LocalStorage`): extend `BaseComponent`, use it with `get(Name)`.
...
1. Path in `src/api/client/path/ConduitBasePath.ts` (`PROFILE_FOLLOW = '/profiles/{data1}/follow'`).
...
Now `get(ConduitRestClient).put.users.with(changes)` works. Waiting for eventual consistency: `condition: { operator: 'EQUALS', path: 'profile.following', value: true, retries: 2 }` (each retry costs budget).
...
Use: `get(ConduitRestClient).api.comments.create(slug, 3)`.
