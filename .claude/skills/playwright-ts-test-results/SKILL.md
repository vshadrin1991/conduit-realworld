...
6. **Finish the export** — review every row of `reports/triage/triage-report.json` and apply the result with an overrides file (see **Review the export** below). The export is shared with managers and manual QA, so it is done only when statuses match the evidence and every text reads as plain business language.
