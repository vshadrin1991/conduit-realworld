...
| Per-test artifacts (trace, screenshot, video, `dom.html`, `error-context.md`)                   | `reports/test-results/<test-dir>/`                       | only for failed tests                                         |
...
`reports/test-results/` is wiped at the start of every run, so analyze artifacts before re-running.
...
4. **Dig into evidence only where needed** (cheapest first):
   ...
   - `reports/test-results/<dir>/error-context.md` — an accessibility snapshot of the page at failure time; best for locator failures.
   - `reports/test-results/<dir>/dom.html` — the full page DOM at failure time (UI tests); check the real markup, classes and attributes when healing a locator. `reports/artifacts.json` lists it with the error, screenshot and video of every failed test.
   ...
   - Trace: `npx playwright show-trace reports/test-results/<dir>/trace.zip` (interactive; tell the user the command rather than trying to read the zip).
...
7. **Ask about defects — never file them inside the triage.** When the reviewed export holds at least one row with status `defect` (or a `need to review` row the evidence turned into one), finish by asking whether to write the defect reports now. Use the AskUserQuestion tool when it is available, offer exactly **create the defect reports now** and **not now**, and name the tests they would cover. On a yes, hand over to the [playwright-ts-defect-creation](../../commands/playwright-ts-defect-creation.md) command: it groups the rows by root cause and writes one Jira-style markdown file per defect into `reports/triage/defects/`, with links to the screenshot, video, DOM and page snapshot of each failure. Do not ask when every failure is an `automation bug` or `flaky` — there is nothing to file; say what to fix or re-run instead.
...
<only when at least one row is `defect`: how many and which tests they cover, then the question — create the defect reports now (`/playwright-ts-defect-creation`)?>
```
