...
| `tests/ui/articles.ui.spec.ts` | Hybrid UI spec: sign-in through `LoginPage` in `beforeEach` (describe tagged `AUTH_QUOTA`), page chains, `waitUntilPageLoaded`, `page.button` / `page.text` for locators outside the named maps, native dialog, UI + API verification, `track` for UI-created data |
