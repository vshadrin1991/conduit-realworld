...
| `tests/ui/articles.ui.spec.ts`                | Hybrid UI spec: `get(Session).login()` in `beforeEach`, page steps, `waitUntilPageLoaded`, `page.button` / `page.text` for locators outside the named maps, native dialog, UI + API verification, `track` for UI-created data |
