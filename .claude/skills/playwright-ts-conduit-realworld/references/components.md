...
Read when using or adding page components — element components (`page.input`, `page.button`, `page.checkbox`, `page.radioButton`, `page.text`), native dialogs (`page.confirmation`), page fragments (`page.header`), `LocalStorage` — or the `Interceptor` utility.

## BaseComponent is for page components only

- `BaseComponent` (`src/base/BaseComponent.ts`) is the base of **page components**: classes that act on the rendered page through `this.page` — element components that take a locator per call, page fragments (`Header`), native dialogs (`Confirmation`) and the page's `LocalStorage`. They live in `src/pageObject/components/`.
