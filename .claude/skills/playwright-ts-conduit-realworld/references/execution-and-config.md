...
- `envConfig` (TEST_ENV, BASE_URL, CI, AUTOMATION_KEY), `authConfig` (TEST_USER_EMAIL/PASSWORD, AUTH_DIR, lock, token scheme), `frameworkConfig` (BROWSER, HEADLESS, SLOW_MO, VIEWPORT_*, LOCALE, TIMEZONE, WORKERS, RETRIES, FULLY_PARALLEL, UI_FULLY_PARALLEL, UI_NEW_BROWSER_PER_TEST, *_TIMEOUT, TRACE, VIDEO, SCREENSHOT, LOG_LEVEL, INTERCEPTOR_NETWORK, INTERCEPTOR_CONSOLE, INTERCEPTOR_BODY_MAX), `reportConfig` (REPORTS_DIR, HTML_REPORT_OPEN, ALLURE, JUNIT, ARTIFACTS). `.env.example` lists every variable.
