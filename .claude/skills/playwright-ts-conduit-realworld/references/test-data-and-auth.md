...
- Only the shared test user is reused; tests must not change its credentials or profile. A scenario that mutates a user creates one.
...
- Every sign-in or registration through the UI spends the auth quota (~5 requests / hour per IP): keep signed-in UI tests few and cover logged-in behaviour through the API where the UI is not what is tested.
