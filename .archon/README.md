...
.archon/
  ...
  scripts/     deterministic helpers of bash nodes (workspace-setup.sh, implementation-inputs.mjs, allure-serve.sh, requirements-inputs.mjs, requirements-gate.mjs)
...
3. Archon works only inside a git repository. Every run gets an isolated git worktree; the first bash node of each workflow (`.archon/scripts/workspace-setup.sh`) links `node_modules` and copies `.env`, `.auth` and `.archon/.env` into it, because a worktree checks out tracked files only.
4. Project setup as usual: `npm install && npx playwright install chromium`, `.env` from `.env.example`.
5. Copy `.archon/.env.example` to `.archon/.env` and set `PROJECT_PATH` to the absolute path of this project. Archon loads `.archon/.env` into every node; `PROJECT_PATH` is where the worktree takes `node_modules`, `.env` and `.auth` from. Bash nodes run in the worktree, not in `PROJECT_PATH`. When it is unset the main checkout is read from git.

## Usage

```bash
archon workflow list
archon workflow run playwright-tests-implementation "tasks/AQA-20260913-settings/AQA-20260913-settings.md"
archon workflow run playwright-tests-review "tests/ui/articles.ui.spec.ts src/pageObject/pages/ArticlePage.ts"
archon workflow run playwright-tests-execution "tests/ui/articles.ui.spec.ts --project=ui"
archon workflow run playwright-requirements-testing -- --r requirements/REQ-01-registration-and-sign-in.md
```

From another directory, pass the project path with `--cwd` (Archon reads `.archon/` and `.archon/.env` from there):

```bash
archon workflow run playwright-tests-execution --cwd <PROJECT_PATH> "tests/ui/articles.ui.spec.ts --project=ui"
```
...
archon workflow run playwright-tests-implementation -- --r requirements/REQ-01-registration-and-sign-in.md --tc tasks/AQA-1/test-cases.md --a tasks/AQA-1/pages
archon workflow run playwright-tests-implementation -- Automate the sign-in errors --tc Unknown email shows: Email not found sign in first
```
...
- Reports are written to the run's artifacts directory (`~/.archon/workspaces/<owner>/<repo>/artifacts/runs/<run-id>/`): `plan.md`, `changed-files.txt`, `implementation-report.md`, `review.md`, `playwright-output.txt`, `execution-report.md`, `allure-serve.log` and the whole `reports/` folder of the run (`reports/allure-results/`, `reports/html/`, `reports/test-results/` with traces, `reports/results.json`). The run works in a throwaway worktree, so this copy is the only one that outlives it. `archon workflow runs` and `archon workflow get <run-id>` show past runs; `archon workflow resume <run-id>` continues a failed one.
- **Allure.** Every workflow has an `allure` bash node (`.archon/scripts/allure-serve.sh`): it copies the `reports/` folder of the run to `reports/` in the run's artifacts and runs `allure serve` on the copied `reports/allure-results` (in Allure 3 `serve` and `open` are the same command: generate, serve, open in the default browser). The server is started with `nohup` so it outlives the node, and the URL is opened in the browser explicitly, because a workflow node has no terminal of its own; the URL is in the node output and in the workflow report. The implementation workflow serves its verification run (skipped when the plan is `BLOCKED`), the execution workflow the run with healed re-runs, the review workflow runs no tests and falls back to the last run in `PROJECT_PATH`. The server keeps running after the workflow; the next `allure` node stops it (pid in `<artifacts>/runs/allure-serve.pid`). To reopen an old run: `npx allure serve <artifacts>/runs/<run-id>/reports/allure-results`.
