...
test.describe('Articles API', () => {
  test('creates an article', async ({ get }) => {
    ...
    const article = await get(ConduitRestClient).post.articles.with(data);
  ...
  test('returns an article by slug to a guest', async ({ get }) => {
    ...
    const article = await get(ConduitRestClient, { guest: true }).get.articles.bySlug(created.slug);
  ...
  test('lists articles filtered by author', async ({ get }) => {
    ...
    const { articles } = await get(ConduitRestClient, { guest: true }).get.articles.list({
  ...
  test('deletes an article', async ({ get }) => {
    ...
    await get(ConduitRestClient).delete.articles.by(created.slug);
  ...
  test('requires a title', async ({ get }) => {
    ...
    const response = await get(ConduitRestClient).response({
