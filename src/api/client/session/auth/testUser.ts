...
import { ConduitRestClient } from '../../ConduitRestClient';
...
async function resolveTestUser(context: APIRequestContext): Promise<TestUser> {
  ...
  if (cached && cacheMatchesEnv) {
    const response = await new ConduitRestClient(context, cached.token).response({
  ...
  const guest = new ConduitRestClient(context);
